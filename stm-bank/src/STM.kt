...
class TxVar<T>(initial: T) {
    ...
    fun openIn(tx: Transaction, update: (T) -> T): T {
        while (true) {
            ...
            val curValue = curLoc.vall(tx) { owner -> owner.abort() }
            if (curValue == TxStatus.ACTIVE) {
                continue
            }
            @Suppress("UNCHECKED_CAST") val updValue = update(curValue as T)
...
private class Loc<T>(
    ...
    fun vall(tx: Transaction, onActive: (Transaction) -> Unit): Any? = if (owner == tx) { newValue } else
        when (owner.status) {
